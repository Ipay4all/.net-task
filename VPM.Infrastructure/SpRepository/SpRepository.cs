...
using static VPM.Models.Constant.Constants;

namespace VPM.Infrastructure.SpRepository
{
    public static class SpRepository
    {
        ...
        public static async Task<string> ExecuteStoreProcedureWithEntitySID(this SpContext catalogDbContext, string sqlQuery, object[] param)
        {
            ...
            if (!string.IsNullOrEmpty(result.ErrorMessage))
            {
                ...
                //    {
                //        var codes = MessageHelper.ReadCommonCodesMessage(result.ErrorMessage, RequestStatus: ErrorMessages.NotFound);
                ...
                //else if (result.Result == DatabaseErrorCodes.NotAllowed)
                //{
                //    var codes = MessageHelper.ReadCommonCodesMessage(result.ErrorMessage, RequestStatus: ErrorMessages.NotAllowed);
                //    throw new HttpStatusCodeException(StatusCodes.Status400BadRequest, codes.Message, codes.Value);
                //}
                //throw new HttpStatusCodeException(StatusCodes.Status500InternalServerError, result.ErrorMessage);                //if (result.Result == DatabaseErrorCodes.NotExist) //not exist
                //{
                //    if (string.IsNullOrWhiteSpace(result.EntitiySID))
                //    {
                //        var codes = MessageHelper.ReadCommonCodesMessage(result.ErrorMessage, RequestStatus: ErrorMessages.NotFound);
