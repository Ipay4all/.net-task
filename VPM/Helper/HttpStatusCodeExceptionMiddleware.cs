...
namespace VPM.Helper
{
    ...
    public class HttpStatusCodeExceptionMiddleware
    {
        ...
        public async Task Invoke(HttpContext context)
        {
            try
            {
                context.Request.Headers.TryGetValue("Accept-Language", out var locale);

                if (!string.IsNullOrEmpty(locale) && (locale.Contains("en") || locale.Contains("ar") || locale.Contains("af")))
                {
                    CultureInfo ci = new CultureInfo(locale);
                    Thread.CurrentThread.CurrentCulture = ci;
                    //Thread.CurrentThread.CurrentUICulture = ci;
                    ValidatorOptions.LanguageManager.Culture = ci;
                }
                else
                {
